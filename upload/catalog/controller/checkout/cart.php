...
class ControllerCheckoutCart extends Controller {
    public function index() {
        // проверка наличия полей в POST-запросе
        $orderData = array(
            ...
            'call_before'   => isset($this->request->post['call_before']) ? true : false,
        ...
        // отправка данных в retailCRM
        $this->sendToRetailCRM($orderData);
	...
	private function sendToRetailCRM($orderData) {
        $crmUrl = 'https://yourcrmurl';
        $apiKey = 'yourapikey';

        $ch = curl_init("$crmUrl/api/v5/orders/create?apiKey=$apiKey");
        ...
        // Обработка ответа от CRM (при необходимости)
        $responseDecoded = json_decode($response, true);
        if (isset($responseDecoded['error'])) {
            // Логика обработки ошибок
            $this->log->write('Error sending data to retailCRM: ' . $responseDecoded['error']);
        }
    }
	public function add() {
		$this->load->language('checkout/cart');
