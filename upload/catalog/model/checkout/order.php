...
class ModelCheckoutOrder extends Model {
    public function addOrder($data) {
        //Добавление заказа, вставка и обновление данных заказа
...
date_added = NOW(), 
date_modified = NOW(),
delivery_date = '" . (isset($data['delivery_date']) ? $this->db->escape($data['delivery_date']) : '') . "', 
delivery_time = '" . (isset($data['delivery_time']) ? $this->db->escape($data['delivery_time']) : '') . "', 
call_before = '" . (isset($data['call_before']) ? (int)$data['call_before'] : 0) . "'
");

// устанавливаем статус заказа в "New"
$this->updateOrderStatusToNew($this->db->getLastId());
	...
	public function updateOrderStatusToNew($order_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id = '1', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	}
...
private function sendOrderToRetailCRM($order_id, $data) {
    ...
    if ($http_code != 200) {
        error_log('CRM API Error: HTTP Code ' . $http_code . ' - ' . $response);
    } elseif ($error) {
        error_log('CRM API Error: ' . $error);
    } else {
