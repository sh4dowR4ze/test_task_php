        ...
        foreach ($response->history as $change) {
            if (isset($change->order)) {
                ...
                if ($orderId) {
                    ...
                    if ($stmt === false) {
                        die('Ошибка подготовки запроса (SELECT): ' . $mysqli->error);
                    }
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($stmt === false) {
                            die('Ошибка подготовки запроса (UPDATE): ' . $mysqli->error);
                        }
                        ...
                        $stmt = $mysqli->prepare("INSERT INTO oc_order (order_id, delivery_date, tracking_number, date_added) VALUES (?, ?, ?, NOW())");
                        if ($stmt === false) {
                            die('Ошибка подготовки запроса (INSERT): ' . $mysqli->error);
                        }
                    ...
                    echo "Пропущен заказ, так как отсутствует идентификатор.\n";
                }
            } else {
